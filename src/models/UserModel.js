...
const toRow = (row) => row ? {
  ...
  can_view_dispatch:     !!row.can_view_dispatch,
...
exports.create = async ({ name, email, password, role = 'reporter', is_active = true, can_edit_reports = false, can_view_tracking = false, can_view_dispatch = false, can_create_cases = true, can_edit_cases = true, can_download_evidence = true, case_access = null, driver_access = null, case_specific_access = null, tracking_page_access = null }) => {
  const [result] = await db.query(
    'INSERT INTO users (name, email, password, role, is_active, can_edit_reports, can_view_tracking, can_view_dispatch, can_create_cases, can_edit_cases, can_download_evidence, case_access, driver_access, case_specific_access, tracking_page_access) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [
      name, email, password, role, is_active ? 1 : 0, can_edit_reports ? 1 : 0,
      can_view_tracking ? 1 : 0, can_view_dispatch ? 1 : 0,
