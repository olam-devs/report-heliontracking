 ...
 * Never blocks requests by default; if accessHours block the user, req.user is omitted.
 */
module.exports = function optionalUser(req, _res, next) {
  try {
    ...
    if (!u || u.active === false) return next();
    if (!isWithinAccessHours(u.accessHours)) return next();
    const safeUser = {
      id: u.id,
      username: u.username,
      role: u.role,
